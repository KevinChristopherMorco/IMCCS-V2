...
if (isset($_POST['user_id'])) {
    ...
    $checkUsername = mysqli_query($mysqli, "SELECT username from admin_profile_tbl WHERE username='$username'");
    ...
    /*$query = "UPDATE user set  user.username='" . $username . "', prfl.fname='" . $fname . "', prfl.lname='" . $lname . "' FROM admin_tbl as user
...
INNER JOIN admin_profile_tbl as prfl ON user.user_id = prfl.user_id
SET user.username = $username, prfl.fname = $fname, prfl.lname = $lname
WHERE user.user_id ";*/



    /*$query = "UPDATE admin_tbl set  username='" . $username . "' WHERE user_id='" . $_POST['user_id'] . "'"; // update form data from the database */

    /*

    $query = "UPDATE admin_tbl as user
INNER JOIN admin_profile_tbl as prfl ON user.user_id = prfl.user_id  set  prfl.fname='" . $fname . "', prfl.lname='" . $lname . "' , prfl.institution='" . $institution .  "' , prfl.updated_at='" . $timestamp . "',
...
*/
    ...
    if ($returnCheckInstitution == 1) {
        ...
        INNER JOIN admin_profile_tbl as prfl ON user.user_id = prfl.user_id  set  prfl.fname=?, prfl.lname=? , prfl.gender=? , prfl.institution=? , prfl.updated_at=?,
